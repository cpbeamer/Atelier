...
const {
  ...
  notifyAgentStart,
  notifyAgentComplete,
  startRunAgentRuntime,
  stopRunAgentRuntime,
  useRunAgentRuntimeService,
  bootstrapAgentRuntimeWorktree,
...
export async function autopilotWorkflow(input: AutopilotInput): Promise<AutopilotOutput> {
  ...
  let runRuntimeServiceStarted = false;
  try {
  ...
  // If the chosen runtime needs a per-run service, start it now.
  if (await useRunAgentRuntimeService()) {
    await bootstrapAgentRuntimeWorktree({ worktreePath });
    await startRunAgentRuntime({ runId, worktreePath });
    runRuntimeServiceStarted = true;
  }

  // Phase 1: Repository Analysis
  await notifyAgentStart({ agentId: 'researcher', agentName: 'Research Agent', terminalType: 'terminal', runId });
  const repoAnalysis = await researchRepo({ projectPath, userContext, agentId: 'researcher', runId });
  await notifyAgentComplete({ agentId: 'researcher', status: 'completed', output: summarize(repoAnalysis), runId });

  // Phase 2: Roadmap Debate (debate-a + debate-b run in parallel inside the activity)
  await notifyAgentStart({ agentId: 'debate-a', agentName: 'Debate Signal', terminalType: 'terminal', runId });
  await notifyAgentStart({ agentId: 'debate-b', agentName: 'Debate Noise', terminalType: 'terminal', runId });
  ...
  await notifyAgentComplete({ agentId: 'debate-a', status: 'completed', output: summarize(approvedFeatures), runId });
  await notifyAgentComplete({ agentId: 'debate-b', status: 'completed', output: summarize(approvedFeatures), runId });

  // Phase 3: Ticket Generation
  await notifyAgentStart({ agentId: 'ticket-bot', agentName: 'Ticket Bot', terminalType: 'direct-llm', runId });
  const { tickets } = await generateTickets({ approvedFeatures, agentId: 'ticket-bot', runId });
  await notifyAgentComplete({ agentId: 'ticket-bot', status: 'completed', output: summarize(tickets), runId });
  if (tickets.length === 0) {
    return {
      status: 'stalled',
      ticketsCreated: 0,
      stalledReason: 'no tickets generated from research/debate output',
    };
  }

  // Phase 4: Scope & Plan
  await notifyAgentStart({ agentId: 'architect', agentName: 'Architect', terminalType: 'terminal', runId });
  const { scopedTickets } = await scopeArchitecture({ tickets, projectPath, worktreePath, agentId: 'architect', runId });
  await notifyAgentComplete({ agentId: 'architect', status: 'completed', output: summarize(scopedTickets), runId });
  if (scopedTickets.length === 0) {
    return {
      status: 'stalled',
      ticketsCreated: 0,
      stalledReason: 'architect produced no scoped tickets',
    };
  }
  ...
  for (const ticket of scopedTickets) {
    await notifyAgentStart({ agentId: 'developer', agentName: 'Developer', terminalType: 'terminal', runId });
    ...
    for (let testLoop = 0; testLoop < 3 && !testsPassed; testLoop++) {
      await notifyAgentStart({ agentId: 'tester', agentName: 'Tester', terminalType: 'terminal', runId });
  ...
  await notifyAgentStart({ agentId: 'verifier', agentName: 'Verifier', terminalType: 'direct-llm', runId });
  ...
  // Phase 8: Push
  await notifyAgentStart({ agentId: 'pusher', agentName: 'Pusher', terminalType: 'direct-llm', runId });
  ...
  return {
    ...
    if (runRuntimeServiceStarted) {
      try {
        await stopRunAgentRuntime({ runId });
      } catch {
